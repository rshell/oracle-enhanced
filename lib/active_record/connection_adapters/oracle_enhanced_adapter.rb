  ...
  module ConnectionAdapters #:nodoc:
    ...
    class OracleEnhancedAdapter < AbstractAdapter
      ...
      NATIVE_DATABASE_TYPES = {
        ...
        :float       => { :name => "NUMBER" }, # { :name => "BINARY_FLOAT" },
      ...
      def timezone_corrected_date(raw_value)
        ::ActiveRecord::Base.default_timezone == :utc && raw_value.respond_to?(:utc) ? raw_value.utc : raw_value
      end

      def quoted_date(value)
        timezone_corrected_date(value).strftime("%Y-%m-%d %H:%M:%S")
      end
